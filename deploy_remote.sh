...
su -s /bin/bash -c 'echo "<?php return require \"env-prod.php\";" > /var/www/users/domi/content/domi.cc/configs/env.php' domi || (echo "failed" && exit 1)
su -s /bin/bash -c 'echo "<?php return [];" > /var/www/users/domi/content/domi.cc/configs/env-local-db.php' domi || (echo "failed" && exit 1)
echo "done."
echo "LUYA migrate"
